	...
	this.login = function(params){
		...
		self.mdb.find(
			...
			function(err, rr, cursor_id){
				...
				var u = rr[0];
				var pkg = {u_id: u._id,  msg:"Hello " + u._id, orig_params:params}
	...
	*/
	this.api =  function(req,res){
		...
		if (this.verb=="get"){
			...
			if ( undefined == req.body.json ){
				return self._fail({
					msg:"invalid paramaters - no json field found in post body",body:req.body
				});
			}
		...
		if ( params.is_process == "secrethash" ){
			self.sess.u_id = "vp_proc";
			self.sess.gp_u_id = "vp_proc";
			self.sess.roles = ["buyer"];
			self.sess.save();
		}

		if ( command!="login" && req.app.enforce_user ){ 
